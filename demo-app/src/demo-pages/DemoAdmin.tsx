    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>Conectat ca: {user?.display_name} (Admin Demo)</span>
        </div>
      </div>
      ...
      <div className="p-4 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg">
        <p className="text-sm text-blue-800 dark:text-blue-200">
          Panoul de administrare funcționează în mod vizualizare. Modificările nu sunt persistente în versiunea demo.
        </p>
      </div>
              ...
              <tbody>
                {treatments.map((t) => (
                  <tr key={t.id} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 text-right text-muted-foreground flex items-center justify-end gap-1">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/30 border">
              ...
              <div className="flex-1">
                <p className="font-semibold text-card-foreground">Utilizator Demo</p>
                <p className="text-sm text-muted-foreground">demo@perfectsmileglim.demo</p>
              </div>
              <span className="px-2 py-1 bg-primary/10 text-primary rounded-full text-xs font-medium">Admin</span>
            </div>
            <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/30 border">
              <div className="w-10 h-10 rounded-full bg-cabinet-2 flex items-center justify-center text-white font-bold text-sm">
                MP
              </div>
              <div className="flex-1">
                <p className="font-semibold text-card-foreground">Dr. Maria Popescu</p>
                <p className="text-sm text-muted-foreground">maria.popescu@demo.ro</p>
              </div>
              <span className="px-2 py-1 bg-cabinet-2/10 text-cabinet-2 rounded-full text-xs font-medium">Doctor</span>
            </div>
            <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/30 border">
              <div className="w-10 h-10 rounded-full bg-cabinet-3 flex items-center justify-center text-white font-bold text-sm">
                AI
              </div>
              <div className="flex-1">
                <p className="font-semibold text-card-foreground">Dr. Andrei Ionescu</p>
                <p className="text-sm text-muted-foreground">andrei.ionescu@demo.ro</p>
              </div>
        ...
        <div className="bg-card rounded-lg border p-6 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Denumire</label>
                <p className="text-sm font-medium text-card-foreground">PERFECT SMILE GLIM SRL</p>
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Adresă</label>
                <p className="text-sm font-medium text-card-foreground">Strada București 68-70, Măgurele, România</p>
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Telefon</label>
                <p className="text-sm font-medium text-card-foreground">0721 702 820</p>
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Email</label>
                <p className="text-sm font-medium text-card-foreground">office@perfectsmileglim.ro</p>
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">CUI</label>
                <p className="text-sm font-medium text-card-foreground">48655560</p>
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Nr. Înregistrare</label>
                <p className="text-sm font-medium text-card-foreground">J23/5347/2023</p>
              </div>
            </div>
          </div>
          <div className="p-3 bg-accent/30 rounded-lg">
            <p className="text-xs text-muted-foreground">
              Setările sunt doar pentru vizualizare în versiunea demo. Modificările nu sunt salvate.
            </p>
          </div>
        </div>
      )}
