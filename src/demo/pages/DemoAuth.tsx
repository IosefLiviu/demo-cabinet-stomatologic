        ...
        <div className="bg-card rounded-xl border shadow-lg p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="Utilizator demo" />
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Parolă</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="Parolă demo" />
            </div>
            <button type="submit" className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity">Conectare Demo</button>
          </form>
          <p className="text-xs text-center text-muted-foreground mt-4">Apăsați "Conectare Demo" cu orice credențiale - autentificarea este simulată</p>
        </div>
      </div>
    </div>
  );
