...
export function DemoDataProvider({ children }: { children: ReactNode }) {
  ...
  const addAppointment = (apt: Omit<DemoAppointment, "id">) => {
    const newApt: DemoAppointment = { ...apt, id: `apt-demo-${Date.now()}` };
  ...
  return (
    <DemoDataContext.Provider
      value={{ patients, doctors, treatments, appointments, stock, addAppointment, updateAppointmentStatus, addPatient }}
    >
      {children}
    </DemoDataContext.Provider>
  );
